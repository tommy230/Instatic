...
| CSS value sanitiser (leaf)   | `src/core/css-sanitize/*`             | Single canonical `sanitiseCssValue` — dependency-free leaf shared by `@core/publisher` and `@core/framework`; drops case-insensitive `</style` or `</script` prefixes (also allowing whitespace after `</`) regardless of what follows, while allowing other `</` sequences such as inline SVG `data:` URIs. `sanitizeModuleCSS` also neutralises `</style` over the assembled stylesheet as defence in depth. |
