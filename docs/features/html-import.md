...
| `input` | `base.input`, `base.checkbox`, `base.radio`, `base.submit`, or `base.button` | Native form attrs modeled by the target module; checkbox/radio runtime attrs such as `data-*` and ARIA are preserved | No |
...
- **Checkbox and radio runtime hooks survive rebuilding.** Safe attributes outside the module-owned set, including `data-*` and ARIA attributes, travel in `htmlAttributes`. This preserves the element contract used by third-party consent and form scripts while the module continues to own `type`, `name`, `id`, `value`, and boolean state.
