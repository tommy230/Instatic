...
Native videos without a source retain their playback attributes, poster, and preload setting. This supports themes that assign the video URL after the page loads, without emitting an empty `src`.
...
- **Direct text inside a recursing container is preserved.** The walker iterates `childNodes` (not just `children`): element children route through the rules, and each significant text node becomes a synthesized `base.text` child with `tag: 'none'` in document order. That no-wrapper text mode publishes back to bare text, so `<div class="num">98%</div>` keeps its original DOM shape. Text-only `<li>` elements map directly to `base.text` with `tag: 'li'`, preserving the same markup while making the list item selectable and inline-editable. List items containing nested markup remain recursing containers. Whitespace runs between elements, including newline-only indentation, collapse to a single bare text-node space. The walker cannot infer block layout without CSS, so it preserves separators and wrapping opportunities for inline content. Parent-edge whitespace is trimmed; `<pre>` content stays verbatim.
...
- **Anchor attribute presence is preserved.** A plain `<a>` without `href` or `target` stores `null` for that prop, and `base.link` omits the attribute when publishing. An explicitly authored `href=""` stays an empty string and publishes as `href=""`. This distinction keeps script-controlled anchors such as menu toggles from acquiring an unintended current-page navigation.
...
| Whitespace-only text (newlines/indentation between tags) | Collapsed between elements; preserved verbatim inside `<pre>` | Interior whitespace becomes a bare text-node space so inline elements retain gaps and wrapping opportunities. Browser layout ignores it between blocks and flex/grid items. Parent-edge whitespace is trimmed. |
