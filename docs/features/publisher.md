...
- After module CSP sources are merged, `ensureSelfInFetchDirectives` restores `'self'` on newly created fetch directives that would otherwise replace the `default-src` fallback; base directives and a new directive set to exactly `'none'` remain closed.
