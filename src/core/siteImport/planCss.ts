 ...
 *
 * A WordPress page prints the theme's customizer CSS, block-support CSS and
 * similar site-wide blocks into every page's head. Each page's `<style>` CSS is
 * parsed as its own source, so a site-wide block became one copy of every rule
 * per page, interleaved through the global sheet in page order. Each copy is
 * harmless on its own, but together they outrank any single page's override of
 * the same selector at equal specificity: the home page of fredschnider.com
 * sets `.main-navigation li a { color:#1e1e1e }` after the site-wide
 * `{ color:#fff }`, and the 76 later pages' copies of the white rule came after
 * it in the flattened sheet, so the store painted the home nav white. A browser
 * sees one copy per page, and the page's own override last. Keeping the first
 * copy and dropping the rest gives the flattened sheet the same shape.
 *
