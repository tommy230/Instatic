...
function collectStyleCssFrom(html: string): string {
  return importHtml(html).styleCss
}
...
describe('collectStyleCss — <style> elements', () => {
  ...
  it('closes an unterminated comment or string so the next block stays live CSS', () => {
    expect(collectStyleCssFrom('<style>.a{color:red}/* open</style><style>.b{color:blue}</style>'))
      .toBe('.a{color:red}/* open*/\n.b{color:blue}')
    expect(collectStyleCssFrom('<style>.a{content:"x</style><style>.b{color:blue}</style>'))
      .toBe('.a{content:"x"}\n.b{color:blue}')
  })

  it('leaves balanced blocks untouched, braces inside comments and strings ignored', () => {
    const css = '.a{content:"{"}/* { */.b{color:blue}'
    expect(collectStyleCssFrom(`<style>${css}</style>`)).toBe(css)
  })
})
